		...
		$.post(base_url + '/ajax/admin_get_video', { video_id: video_id },
			function (response) {
				if (response.status) {
					...
					if (response.channel) selectedChannels.push(response.channel);
					if (response.channel2) selectedChannels.push(response.channel2);
					if (response.channel3) selectedChannels.push(response.channel3);
					if (response.channel4) selectedChannels.push(response.channel4);
					$('#edit-category').val(selectedChannels).trigger('change');
